...
const slice = createSlice({
  ...
  reducers: {
    ...
    saveUserManagedClubs(
      state: userProfileState,
      action: PayloadAction<{ managedClub: Partial<ClubSimInfo> }>
    ) {
      const { managedClub } = action.payload;
      const array: any = state.userProfileData.managedClubs;
      array.push({ ...managedClub });
      state.userProfileData.managedClubs = array;
    },

    editUserManagedClubs(
      state: userProfileState,
      action: PayloadAction<{ managedClub: Partial<ClubSimInfo> }>
    ) {
      const { managedClub } = action.payload;
      ...
      const array: any = state.userProfileData.managedClubs;
      const idx = array.findIndex((club) => club.clubid === managedClub.clubid);
      array[idx] = managedClub;
      state.userProfileData.managedClubs = [...array];
    },
    getUserManagedClubs(
      state: userProfileState,
      ...
      state.userProfileData.managedClubs =
        state.userProfileData.managedClubs[clubid];
    },
    updateUserRole(
      state: userProfileState,
...
export const saveUserManagedClubList =
  (params: { managedClub: Partial<ClubSimInfo> }): AppThunk =>
  (dispatch) => {
    try {
      dispatch(
        slice.actions.saveUserManagedClubs({ managedClub: params.managedClub })
      );
    } catch (err) {
      console.log(err);
    }
...
export const editUserManagedClubList =
  (params: { managedClub: Partial<ClubSimInfo> }): AppThunk =>
  (dispatch) => {
    try {
      dispatch(
        slice.actions.editUserManagedClubs({ managedClub: params.managedClub })
      );
    } catch (err) {
      console.log(err);
    }
...
export const getUserManagedClubList =
  (params: { clubid: string }): AppThunk =>
  (dispatch) => {
    try {
      dispatch(slice.actions.getUserManagedClubs({ clubid: params.clubid }));
    } catch (err) {
      console.log(err);
    }
  };
  ...
  async (dispatch) => {
    try {
      const response = await axios.post(
        "/api/club/deactivate/" + params.clubid
      );
      if (response.status === 200) {
